...
# ethereum-etl 安装
etl_install() {
    python3 -m pip install cython cytoolz sqlalchemy pg8000 || RED 'python3 pip install exec failed'
    python3 -m pip install ethereum-etl || RED 'ethereum-etl install exec failed'
    ethereumetl > /dev/null && GREEN 'ethereum-etl install successed'
}
...
erigon_install() {
    ...
    ./build/bin/erigon --help > /dev/null && GREEN 'erigon install successed'
    cp ./build/bin/erigon "$root_dir/bin/"
}

# postgresql 初始化
postgresql_init() {
    python3 "$pwd_dir"/postgresql_init.py
}